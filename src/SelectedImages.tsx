...
export const SelectedImages: FC<Props> = props => (
  <div className="selected">
    {props.images.map(image => (
      <div key={image.public_id} className="asset-thumbnail">
        <div className="asset-preview">
          <div className="asset-thumbnail__actions-pane">
            {image.secure_url && (
              <a className="asset-action" title="Download" href={image.secure_url} target="_blank" rel="noreferrer">
                <i className="icon-arrow-down-line" />
              </a>
            )}
            {!props.isDisabled && (
              <button
                className="asset-action asset-action--remove" title="Remove"
...
const renderImage = (image: CloudinaryImage) => image.secure_url
  ? (
    <a href={image.secure_url} target="_blank" rel="noreferrer">
      <img className="asset-thumbnail__image" src={image.secure_url} alt={image.public_id} />
    </a>
  )
  : (
